...
#
# Upotreba:
#   ./start_cluster.sh RATE BATCH TRAJANJE BROJ_WORKERA
#
# BROJ_WORKERA:
#   1 = pi5-16
#   2 = pi5-16 + pi5-8
...
#   ./start_cluster.sh 1000 5 60 2
#   ./start_cluster.sh 500 20 60 4
...
echo "[3/8] Čistim prethodni test..."

pkill -f '[g]enerator.py' 2>/dev/null || true
...
kill -0 "$SERVER_PID" 2>/dev/null \
    || fail "server.py nije aktivan."

echo "[5/8] Pokrećem workere..."
...
kill -0 "$LOCAL_WORKER_PID" 2>/dev/null \
    || fail "worker na pi5-16 nije aktivan."
...
for host in "${ACTIVE_REMOTE[@]}"; do
    ...
    then
        ...
        timeout 8 ssh -n \
            ...
            -o ConnectTimeout=5 \
            "$host" \
            "tail -20 /tmp/monitorko_worker.log 2>/dev/null || true"

        fail "worker na $host nije aktivan."
    fi
done

echo "[6/8] Pokrećem generator.py..."

python3 "$PROJECT/generator.py" "$RATE" \
...
kill -0 "$GENERATOR_PID" 2>/dev/null \
    || fail "generator.py nije aktivan."
...
#
# Brisanje metrics.csv nakon zagrijavanja.
# server.py otvara datoteku ponovno za svako zapisivanje,
# pa će sljedeća metrika automatski napraviti novi CSV i header.
#
rm -f "$PROJECT/metrics.csv"
...
if [[ -f "$PROJECT/metrics.csv" ]]; then
    ...
    echo "Generator log:"
    echo "  $LOGS/generator.log"
    echo

    echo "Server log:"
    echo "  $LOGS/server.log"
else
    echo "GRESKA: metrics.csv nije pronađen."
    exit 1
fi
